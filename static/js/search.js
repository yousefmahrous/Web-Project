...
function initSearchPage() {
    ...
    form.addEventListener("submit", (e) => handleSearch(e, form, resultsContainer, token));
}

async function loadCategories(datalist) {
    try {
        const response = await fetch("http://127.0.0.1:8000/api/books/categories"); // placeholder API
...
async function handleSearch(event, form, resultsContainer, token) {
    ...
    try {
        const books = await fetchBooks(title, author, category, token);
...
async function fetchBooks(title, author, category, token) {
    const url = `/api/books/search?title=${encodeURIComponent(title)}&author=${encodeURIComponent(author)}&category=${encodeURIComponent(category)}`;
